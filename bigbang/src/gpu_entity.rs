...
use super::Dimension;
use crate::{as_entity::AsEntity, Entity, simulation_result::SimulationResult, Node};
        ...
        for opt_node in [&node.left, &node.right].iter()
        {
            if let Some(node) = opt_node {
                if node.points.is_some() {
                    for point in node.points.as_ref().expect("Broken tree structure: unexpected null node") {
